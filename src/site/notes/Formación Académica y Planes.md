---
{"tags":["formación","académico","planes","UANL","biotecnología"],"tipo":"formación","dg-publish":true,"permalink":"/formacion-academica-y-planes/","dgPassFrontmatter":true,"created":"2026-05-05T14:27:12.026-07:00","dg-note-properties":{"tags":["formación","académico","planes","UANL","biotecnología"],"tipo":"formación"}}
---


# Formación Académica y Planes

> ⚠️ **Pendiente** — faltan detalles sobre materias, proyectos escolares y el roadmap de posgrado por definir mejor.

## Trayectoria escolar

### Educación básica y secundaria
- Educación convencional; aprendizaje técnico mayoritariamente autodidacta en paralelo

### Preparatoria — Prepa 3, UANL (2023–2026)
- Grupo: 305
- Ciclo actual: Enero–Junio 2026
- Actividades extracurriculares: Robótica, Laboratorio de biología, Torneos de física
- Asesor clave: Dr. Gerardo (laboratorio de biología)

## Aprendizaje autodidacta paralelo (cronológico)
| Período | Áreas |
|---|---|
| ~2015–2018 | Lua, scripting, modificación de sistemas |
| ~2019–2021 | Ciberseguridad, ingeniería inversa, automatización |
| ~2021–2023 | Hardware, electrónica, reparación de equipos |
| 2023–presente | Robótica (C++, Python, PID, ESP32, OpenMV) |
| 2025–presente | Biología molecular, fitoquímica, protocolos de laboratorio |
| 2026–presente | Bioinformática, simulación metabólica (COPASI, tellurium) |

## Planes académicos

### Corto plazo (2026–2027)
- Asistir a torneo de robotica robofut en cdmx julio 2026 junto torneo de robotica super regional en canada toronto octubre 2026
- Finalizar bachillerato
- Preparar admisión a LBG, UANL
- Publicar primer preprint / contribución científica en biogerontología computacional (objetivo personal)
- Aprender: Hallmarks of Aging, Uri Alon (Systems Biology), EDOs, tellurium, BioModels

### Mediano plazo (2027–2031)
- **Licenciatura en Biotecnología Genómica (LBG)**
- Facultad de Ciencias Biológicas, UANL
- Enfoque: biología computacional, biología sintética, biogerontología

### Largo plazo (2031+)
- Maestría y Doctorado en Ciencias (PhD)
- Facultad de Medicina, UANL (potencial)
- Objetivo: investigación en biogerontología y medicina regenerativa
- Meta científica: contribuir a reversión de efectos del envejecimiento a nivel molecular
