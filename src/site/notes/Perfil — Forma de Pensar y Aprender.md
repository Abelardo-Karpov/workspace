---
{"tags":["perfil","meta","identidad","forma-de-pensar"],"tipo":"perfil","dg-publish":true,"permalink":"/perfil-forma-de-pensar-y-aprender/","dgPassFrontmatter":true,"created":"2026-05-05T14:27:12.464-07:00","dg-note-properties":{"tags":["perfil","meta","identidad","forma-de-pensar"],"tipo":"perfil"}}
---


# Perfil — Forma de Pensar y Aprender

> ⚠️ **Pendiente** — faltan reflexiones personales, más ejemplos del estilo cognitivo y referentes intelectuales por agregar.

## Estilo cognitivo
- Pensamiento sistémico: conecta conceptos de distintas disciplinas como nodos de un grafo
- Aprendizaje por inmersión y prueba-error antes que por instrucción formal
- Identifica nodos centrales de información; expande por asociación estructural
- Busca entender *cómo funciona por dentro*, no solo *cómo se usa*

## Patrón de aprendizaje histórico
> Roblox → Lua → Ingeniería inversa → Ciberseguridad → Hardware → Robótica → Biología

En cada transición: entró sin guía, construyó comprensión desde los fundamentos, llegó a nivel funcional-avanzado.

## Lo que lo frena (autodiagnóstico)
- Períodos de baja motivación o estado mental no óptimo
- Interrupciones externas (trabajo, torneos)
- Scope demasiado grande antes de dominar la base

## Lo que lo impulsa
- Preguntas que no tienen respuesta obvia
- Sistemas complejos con lógica interna descubrible
- Retos técnicos concretos con resultado medible

## Intereses intelectuales / culturales, faltan
- **Ficción:** Steins;Gate, Subete ga f ni naru
- **Ciencia:** López-Otín (Hallmarks), Grigori Perelman (rigor matematico como ideal), Leonardo Da Vinci, Newton
- **Filosofía previa:** Jacobo Grinberg, metafísica, ocultismo (fase explorada, no activa)

## Preferencias de interacción
- Responde mejor a retroalimentación directa y honesta
- Aprende mejor cuando ve la conexión entre piezas antes del detalle
